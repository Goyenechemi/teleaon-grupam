...
function VideoPlaceholder({
  ...
  return (
    <div
      className={`relative rounded-[32px] md:rounded-[48px] bg-gradient-to-br from-primary-container/20 to-surface-container border border-outline-variant/30 flex items-center justify-center group cursor-pointer overflow-hidden ${className}`}
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,_rgba(255,218,215,0.15)_0%,_transparent_60%)] group-hover:bg-[radial-gradient(circle_at_50%_50%,_rgba(255,218,215,0.3)_0%,_transparent_60%)] transition-all duration-500" />
      <Play className={`${iconSize} text-primary z-10 transform group-hover:scale-110 transition-transform duration-500 opacity-80 group-hover:opacity-100`} />
      <div className="absolute bottom-6 left-6 font-label-md text-label-md text-primary uppercase bg-background/50 backdrop-blur-md px-4 py-2 rounded-full transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
        {label}
      </div>
    </div>
  );
